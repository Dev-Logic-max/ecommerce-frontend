"use client"

import type React from "react"
import { useState } from "react"
import Link from "next/link"
import { usePathname } from "next/navigation"
import { useTheme } from "@/components/theme/AdminsThemeProvider"
import { cn } from "@/lib/utils"
import { Button } from "@/components/ui/button"
import { Separator } from "@/components/ui/separator"
import { Badge } from "@/components/ui/badge"
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  AlertDialogContent,
  AlertDialogDescription,
  AlertDialogFooter,
  AlertDialogHeader,
  AlertDialogTitle,
} from "@/components/ui/alert-dialog"

// React Icons
import {
  FiHome,
  FiActivity,
  FiBarChart2,
  FiUsers,
  FiLock,
  FiShoppingBag,
  FiPackage,
  FiBox,
  FiCode,
  FiTerminal,
  FiDatabase,
  FiGitBranch,
  FiSettings,
  FiGlobe,
  FiServer,
  FiShield,
  FiLogOut,
  FiChevronLeft,
  FiChevronRight,
} from "react-icons/fi"
import { GiCubes } from "react-icons/gi"
import { TbBrandVscode } from "react-icons/tb"

interface SidebarItemProps {
  icon: React.ReactNode
  label: string
  href: string
  expanded: boolean
  active?: boolean
  badge?: string
  premium?: boolean
}

const SidebarItem = ({ icon, label, href, expanded, active = false, badge, premium }: SidebarItemProps) => {
  const { themeConfig, sidebarDesignConfig } = useTheme()
  const [isHovered, setIsHovered] = useState(false)

  const getIconDisplay = () => {
    if (sidebarDesignConfig.name === "icons") {
      const iconMap: Record<string, string> = {
        Dashboard: "🏠",
        Activity: "📊",
        Analytics: "📈",
        "User Management": "👥",
        "Role Management": "🔐",
        "Shop Management": "🏪",
        "Product Management": "📦",
        "Order Management": "📋",
        "Warehouse Management": "🏭",
        "API Explorer": "🔧",
        Console: "💻",
        Database: "🗄️",
        Deployments: "🚀",
        "Code Editor": "📝",
        Settings: "⚙️",
        Environment: "🌍",
        Infrastructure: "🏗️",
        Security: "🛡️",
      }
      return <span className="text-lg">{iconMap[label] || "📄"}</span>
    }
    return icon
  }

  const getItemStyles = () => {
    const baseStyles = cn(
      "flex items-center rounded-xl px-3 py-3 text-sm font-medium transition-all duration-300 group relative overflow-hidden",
      expanded ? "gap-3" : "gap-0 w-12 h-12 justify-center",
      "hover:scale-[1.02] transform-gpu",
      !expanded && "mx-1",
    )

    if (sidebarDesignConfig.name === "glass") {
      return cn(
        baseStyles,
        active ? "glass-effect shadow-lg border border-opacity-30 text-white" : "hover:glass-effect hover:shadow-md",
      )
    }

    if (sidebarDesignConfig.name === "minimal") {
      return cn(
        baseStyles,
        active
          ? "bg-gradient-to-r from-primary/20 to-accent/20 border-l-4 border-primary shadow-sm"
          : "hover:bg-muted/50",
      )
    }

    return cn(
      baseStyles,
      active
        ? cn(
            "shadow-lg border border-opacity-30",
            "before:absolute before:left-0 before:top-0 before:bottom-0 before:w-1 before:rounded-r-full before:shadow-lg",
          )
        : "hover:shadow-md",
    )
  }

  const getItemColors = () => {
    if (sidebarDesignConfig.name === "glass") {
      return {
        backgroundColor: active
          ? `linear-gradient(135deg, ${themeConfig.colors.primary}40, ${themeConfig.colors.accent}30)`
          : isHovered
            ? `rgba(255, 255, 255, 0.1)`
            : "transparent",
        color: active || isHovered ? "white" : themeConfig.colors.sidebarText,
        borderColor: active ? `${themeConfig.colors.primary}50` : "transparent",
      }
    }

    if (sidebarDesignConfig.name === "minimal") {
      return {
        backgroundColor: active
          ? `linear-gradient(90deg, ${themeConfig.colors.primary}15, ${themeConfig.colors.accent}10)`
          : isHovered
            ? `${themeConfig.colors.muted}50`
            : "transparent",
        color: active ? themeConfig.colors.primary : themeConfig.colors.sidebarText,
        borderColor: active ? themeConfig.colors.primary : "transparent",
      }
    }

    return {
      backgroundColor: active ? `${themeConfig.colors.primary}20` : "transparent",
      color: active ? themeConfig.colors.primary : themeConfig.colors.sidebarText,
      borderColor: active ? `${themeConfig.colors.primary}30` : "transparent",
    }
  }

  const itemColors = getItemColors()

  return (
    <TooltipProvider delayDuration={100}>
      <Tooltip>
        <TooltipTrigger asChild>
          <Link
            href={href}
            className={getItemStyles()}
            style={{
              background: itemColors.backgroundColor,
              color: itemColors.color,
              borderColor: itemColors.borderColor,
            }}
            onMouseEnter={() => setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
          >
            {/* Enhanced gradient overlay for active state */}
            {active && sidebarDesignConfig.name !== "glass" && sidebarDesignConfig.name !== "minimal" && (
              <>
                <div className="absolute inset-0 opacity-10 rounded-xl" style={{ background: themeConfig.gradient }} />
                <div
                  className="absolute left-0 top-0 bottom-0 w-1 rounded-r-full shadow-lg"
                  style={{ background: themeConfig.gradient }}
                />
              </>
            )}

            {/* Enhanced hover effect for active items */}
            {active && isHovered && sidebarDesignConfig.name !== "glass" && (
              <div
                className="absolute inset-0 opacity-20 rounded-xl transition-opacity duration-300"
                style={{ background: themeConfig.secondaryGradient }}
              />
            )}

            <div className={cn("transition-all duration-200 relative z-10", active && "scale-110")}>
              {getIconDisplay()}
            </div>

            <span
              className={cn(
                "transition-all duration-300 relative z-10",
                expanded ? "opacity-100 w-full" : "opacity-0 w-0 overflow-hidden",
              )}
            >
              {label}
            </span>

            {/* Premium badge */}
            {premium && expanded && (
              <Badge
                className="ml-auto text-xs px-2 py-0.5 font-bold"
                style={{
                  background: themeConfig.secondaryGradient,
                  color: "white",
                }}
              >
                PRO
              </Badge>
            )}

            {/* Static notification badge */}
            {badge && expanded && (
              <div
                className="ml-auto static-badge"
                style={{
                  backgroundColor: themeConfig.colors.danger,
                }}
              >
                {badge}
              </div>
            )}
          </Link>
        </TooltipTrigger>
        {!expanded && (
          <TooltipContent
            side="right"
            className="enhanced-tooltip font-medium px-3 py-2 text-sm"
            style={{
              background: themeConfig.gradient,
              color: "white",
              border: "none",
              boxShadow: "0 10px 25px rgba(0, 0, 0, 0.2)",
            }}
          >
            {label}
            {premium && <span className="ml-2 text-yellow-300">👑</span>}
          </TooltipContent>
        )}
      </Tooltip>
    </TooltipProvider>
  )
}

interface SidebarSectionProps {
  title: string
  expanded: boolean
  children: React.ReactNode
}

const SidebarSection = ({ title, expanded, children }: SidebarSectionProps) => {
  const { themeConfig } = useTheme()

  return (
    <div className="space-y-2">
      {expanded && (
        <div className="px-3 py-2">
          <h3
            className="text-xs font-bold uppercase tracking-wider opacity-70 text-shadow"
            style={{ color: themeConfig.colors.sidebarText }}
          >
            {title}
          </h3>
        </div>
      )}
      <div className="space-y-1">{children}</div>
    </div>
  )
}

interface DeveloperSidebarProps {
  expanded: boolean
  onToggle: () => void
}

export function DeveloperSidebar({ expanded, onToggle }: DeveloperSidebarProps) {
  const [logoutDialogOpen, setLogoutDialogOpen] = useState(false)
  const pathname = usePathname()
  const { themeConfig, sidebarDesignConfig } = useTheme()

  const handleLogout = () => {
    setLogoutDialogOpen(true)
  }

  const confirmLogout = () => {
    console.log("Logging out...")
    setLogoutDialogOpen(false)
  }

  const getSidebarStyles = () => {
    const baseStyles = cn(
      "fixed left-0 top-16 z-30 hidden lg:flex flex-col transition-all duration-500 ease-in-out shadow-2xl sidebar-scrollbar",
      expanded ? "w-64 h-[calc(100vh-64px)]" : "w-20 h-[calc(100vh-100px)] rounded-3xl mx-4 my-4",
      sidebarDesignConfig.className,
    )

    return baseStyles
  }

  const getSidebarBackground = () => {
    if (sidebarDesignConfig.name === "glass") {
      return {
        backgroundColor: "rgba(255, 255, 255, 0.1)",
        backdropFilter: "blur(25px)",
        WebkitBackdropFilter: "blur(25px)",
        border: `1px solid rgba(255, 255, 255, 0.2)`,
        boxShadow: "0 8px 32px rgba(0, 0, 0, 0.1)",
      }
    }

    if (sidebarDesignConfig.name === "minimal") {
      return {
        backgroundColor: themeConfig.colors.background,
        borderRight: `2px solid ${themeConfig.colors.border}`,
        boxShadow: "2px 0 10px rgba(0, 0, 0, 0.05)",
      }
    }

    return {
      backgroundColor: `${themeConfig.colors.sidebar}95`,
      borderColor: themeConfig.colors.sidebarBorder,
      border: `1px solid ${themeConfig.colors.sidebarBorder}`,
    }
  }

  return (
    <>
      <div className={getSidebarStyles()} style={getSidebarBackground()}>
        {/* Header */}
        <div
          className={cn(
            "flex items-center border-b transition-all duration-300",
            expanded ? "px-6 py-5 justify-between" : "px-4 py-5 justify-center",
          )}
          style={{ borderColor: themeConfig.colors.sidebarBorder }}
        >
          <div className="flex items-center gap-3">
            <div
              className="flex items-center justify-center w-10 h-10 rounded-xl shadow-lg"
              style={{ background: themeConfig.gradient }}
            >
              <FiCode className="h-5 w-5 text-white" />
            </div>
            {expanded && (
              <div>
                <h2
                  className="font-bold text-lg text-shadow"
                  style={{ color: themeConfig.colors.sidebarText, textShadow: themeConfig.textShadow }}
                >
                  DevAdmin
                </h2>
                <p className="text-xs opacity-70" style={{ color: themeConfig.colors.sidebarText }}>
                  Developer Portal
                </p>
              </div>
            )}
          </div>
        </div>

        {/* Toggle Button */}
        <Button
          variant="ghost"
          size="icon"
          onClick={onToggle}
          className={cn(
            "absolute h-8 w-8 rounded-full shadow-lg border-2 transition-all duration-300 hover:scale-110",
            expanded ? "-right-4 top-8" : "-right-4 top-12",
          )}
          style={{
            background: themeConfig.gradient,
            borderColor: themeConfig.colors.background,
            color: "white",
          }}
        >
          {expanded ? <FiChevronLeft className="h-4 w-4" /> : <FiChevronRight className="h-4 w-4" />}
        </Button>

        {/* Navigation */}
        <div className={cn("flex-1 overflow-y-auto py-4 space-y-6 sidebar-scrollbar", expanded ? "px-4" : "px-2")}>
          {/* Overview Section */}
          <SidebarSection title="Overview" expanded={expanded}>
            <SidebarItem
              icon={<FiHome className="h-5 w-5" />}
              label="Dashboard"
              href="/dashboard/developer/dashboard"
              active={pathname === "/dashboard/developer/dashboard"}
              expanded={expanded}
            />
            <SidebarItem
              icon={<FiActivity className="h-5 w-5" />}
              label="Activity"
              href="/dashboard/developer/activity"
              active={pathname === "/dashboard/developer/activity"}
              expanded={expanded}
              badge="12"
            />
            <SidebarItem
              icon={<FiBarChart2 className="h-5 w-5" />}
              label="Analytics"
              href="/dashboard/developer/analytics"
              active={pathname === "/dashboard/developer/analytics"}
              expanded={expanded}
              premium
            />
          </SidebarSection>

          <Separator style={{ backgroundColor: themeConfig.colors.sidebarBorder }} />

          {/* System Section */}
          <SidebarSection title="System" expanded={expanded}>
            <SidebarItem
              icon={<FiUsers className="h-5 w-5" />}
              label="User Management"
              href="/dashboard/developer/users"
              active={pathname === "/dashboard/developer/users"}
              expanded={expanded}
            />
            <SidebarItem
              icon={<FiLock className="h-5 w-5" />}
              label="Role Management"
              href="/dashboard/developer/roles"
              active={pathname === "/dashboard/developer/roles"}
              expanded={expanded}
            />
            <SidebarItem
              icon={<FiShoppingBag className="h-5 w-5" />}
              label="Shop Management"
              href="/dashboard/developer/shops"
              active={pathname === "/dashboard/developer/shops"}
              expanded={expanded}
            />
            <SidebarItem
              icon={<FiPackage className="h-5 w-5" />}
              label="Product Management"
              href="/dashboard/developer/products"
              active={pathname === "/dashboard/developer/products"}
              expanded={expanded}
            />
            <SidebarItem
              icon={<FiBox className="h-5 w-5" />}
              label="Order Management"
              href="/dashboard/developer/orders"
              active={pathname === "/dashboard/developer/orders"}
              expanded={expanded}
              badge="5"
            />
            <SidebarItem
              icon={<GiCubes className="h-5 w-5" />}
              label="Warehouse Management"
              href="/dashboard/developer/warehouse"
              active={pathname === "/dashboard/developer/warehouse"}
              expanded={expanded}
            />
          </SidebarSection>

          <Separator style={{ backgroundColor: themeConfig.colors.sidebarBorder }} />

          {/* Development Section */}
          <SidebarSection title="Development" expanded={expanded}>
            <SidebarItem
              icon={<FiCode className="h-5 w-5" />}
              label="API Explorer"
              href="/dashboard/developer/api"
              active={pathname === "/dashboard/developer/api"}
              expanded={expanded}
              premium
            />
            <SidebarItem
              icon={<FiTerminal className="h-5 w-5" />}
              label="Console"
              href="/dashboard/developer/console"
              active={pathname === "/dashboard/developer/console"}
              expanded={expanded}
              premium
            />
            <SidebarItem
              icon={<FiDatabase className="h-5 w-5" />}
              label="Database"
              href="/dashboard/developer/database"
              active={pathname === "/dashboard/developer/database"}
              expanded={expanded}
            />
            <SidebarItem
              icon={<FiGitBranch className="h-5 w-5" />}
              label="Deployments"
              href="/dashboard/developer/deployments"
              active={pathname === "/dashboard/developer/deployments"}
              expanded={expanded}
            />
            <SidebarItem
              icon={<TbBrandVscode className="h-5 w-5" />}
              label="Code Editor"
              href="/dashboard/developer/editor"
              active={pathname === "/dashboard/developer/editor"}
              expanded={expanded}
              premium
            />
          </SidebarSection>

          <Separator style={{ backgroundColor: themeConfig.colors.sidebarBorder }} />

          {/* Configuration Section */}
          <SidebarSection title="Configuration" expanded={expanded}>
            <SidebarItem
              icon={<FiSettings className="h-5 w-5" />}
              label="Settings"
              href="/dashboard/developer/settings"
              active={pathname === "/dashboard/developer/settings"}
              expanded={expanded}
            />
            <SidebarItem
              icon={<FiGlobe className="h-5 w-5" />}
              label="Environment"
              href="/dashboard/developer/environment"
              active={pathname === "/dashboard/developer/environment"}
              expanded={expanded}
            />
            <SidebarItem
              icon={<FiServer className="h-5 w-5" />}
              label="Infrastructure"
              href="/dashboard/developer/infrastructure"
              active={pathname === "/dashboard/developer/infrastructure"}
              expanded={expanded}
              premium
            />
            <SidebarItem
              icon={<FiShield className="h-5 w-5" />}
              label="Security"
              href="/dashboard/developer/security"
              active={pathname === "/dashboard/developer/security"}
              expanded={expanded}
              premium
            />
          </SidebarSection>
        </div>

        {/* Footer */}
        <div
          className={cn("border-t p-4", !expanded && "px-2")}
          style={{ borderColor: themeConfig.colors.sidebarBorder }}
        >
          {/* {expanded && (
            <div
              className="mb-4 p-4 rounded-xl border"
              style={{
                background: `linear-gradient(135deg, ${themeConfig.colors.primary}10, ${themeConfig.colors.accent}05)`,
                borderColor: `${themeConfig.colors.primary}20`,
              }}
            >
              <div className="flex items-center gap-3 mb-2">
                <span className="text-2xl">🚀</span>
                <div>
                  <h4 className="font-semibold text-sm text-shadow" style={{ color: themeConfig.colors.sidebarText }}>
                    Upgrade to Pro
                  </h4>
                  <p className="text-xs opacity-70" style={{ color: themeConfig.colors.sidebarText }}>
                    Unlock premium features
                  </p>
                </div>
              </div>
              <Button
                size="sm"
                className="w-full font-medium btn-gradient"
                style={{
                  background: themeConfig.gradient,
                  color: "white",
                }}
              >
                Upgrade Now
              </Button>
            </div>
          )} */}

          <Button
            variant="ghost"
            onClick={handleLogout}
            className={cn(
              "w-full justify-start text-red-500 hover:text-red-600 hover:bg-red-50 transition-all duration-200",
              !expanded && "justify-center px-0",
            )}
          >
            <FiLogOut className="h-5 w-5" />
            {expanded && <span className="ml-3">Logout</span>}
          </Button>
        </div>
      </div>

      {/* Logout Confirmation Dialog */}
      <AlertDialog open={logoutDialogOpen} onOpenChange={setLogoutDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Confirm Logout</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to logout from the developer dashboard?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={confirmLogout} className="bg-red-500 hover:bg-red-600 text-white">
              Logout
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
  )
}
